...
class CTCaseDataset(data.Dataset):
    ...
    def __init__(
        ...
        # Filter dataset to only include samples where files exist
        valid_indices = []
        missing_samples = []
        for idx, row in dataset.iterrows():
            ...
            if image_path.exists() and metadata_path.exists():
                ...
                missing_samples.append(f"{patient_id}_nodule_{nodule_id}")
        
        self.dataset = dataset.loc[valid_indices].reset_index(drop=True)
    ...
    def __getitem__(self, idx):  # caseid, z, y, x, label, radius
        ...
        # Double-check files exist (safety check)
        ...
        try:
            ...
            raise RuntimeError(f"Error loading {patient_id}_nodule_{nodule_id}: {e}")
        ...
        sample = {
            ...
            "label": target.long(),
            "ID": f"{patient_id}_nodule_{nodule_id}",
            "score": score,
        }
        return sample
