...
class Pulse3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 4) Classify
        # Get the CLS token (at index 0)
        cls_token_out = tokens[:, 0, :]
        
        # Get the average of all patch tokens (from index 1 onwards)
        avg_patch_tokens = torch.mean(tokens[:, 1:, :], dim=1)

        # Combine them by concatenating
        combined_features = torch.cat((cls_token_out, avg_patch_tokens), dim=1) 

        # Pass the combined features to the (modified) head
        return self.head(avg_patch_tokens)
